...
**Iter 77 — glasses: doors+gun textures dropped + very slow → non-evicting WebGPU texture budget (2026-06-15).** User: on the glasses every texture shows EXCEPT the doors and gun(s) (untextured), and it runs very slow; iPhone/desktop fine. ROOT CAUSE = the **non-evicting `r_wgpuTexBudgetMB` budget** (wearable=80MB, `RenderBackend_WebGPU.cpp:1801`). `getEngineTextureView` builds a GPU texture on first DRAW of a surface; once `gpuTexBytes` tops the 80MB cap it returns the checker fallback — and the deny path **does not cache the decision**, so the denied image re-runs its full `malloc`+`boxReduceHalf` rebuild EVERY frame (the slowness). DRAW ORDER explains the door/gun specificity: the world is drawn first and eats the 80MB; the doors (`func_door` movers) and the player view-weapon are drawn LAST in the frame, so they are exactly the surfaces denied → untextured + per-frame rebuild. (At 128px each material's diffuse+bump+specular ≈ 260KB, so the enpro spawn's ~300 visible materials genuinely exceed 80MB.) The 80MB cap only ever existed to dodge the **iOS-Safari whole-tab jetsam** (iter 28, the "82% Starting DOOM 3" crash); the Meta Ray-Ban glasses are an **Android WebView** where that jetsam doesn't apply, so the cap is pure downside there. FIX (d3Runtime.js, JS/cvar only — no engine rebuild): keep iOS pinned at 80MB (`/iPad|iPhone|iPod/` UA, plus the iPadOS-desktop `MacIntel`+maxTouchPoints>1 case) but give every NON-iOS wearable (the glasses) **160MB**; `?texbudget=N` still overrides on either platform. RULED OUT: `image_preload` is NOT the lever — the WebGPU texCache is filled by DRAW REPLAY, not by preload, so preload doesn't change the GPU working set (a prior image_preload-0 hypothesis was reverted; image_preload stays at the iter-73/74 everything-at-boot default). ON-DEVICE VERIFY (glasses): `window.__d3WgpuTexDrop` should fall to 0 (was >0), `window.__d3WgpuTexMB` shows the resident MB, and the console `[d3] WebGPU image FALLBACK (gpu-budget): …` lines for the gun/doors should stop. If 160 still drops, bump with `?texbudget=256`; if the glasses OOM/crash, lower with `?texbudget=120`. (Deeper future fix if the working set ever exceeds a safe ceiling: a real LRU on the budget path + a per-ptr deny cache so the gun, drawn every frame = most-recently-used, is never the one evicted and the deny path stops rebuilding — held; the budget bump is sufficient for the glasses' headroom.)
