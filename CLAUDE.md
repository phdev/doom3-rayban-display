...
**Iter 124 — console "checkerboard" + ammo-panel mask-leak + additive-flash black-bg = ONE bug: the framebuffer ALPHA channel is polluted, so the dst-alpha-gated overlays read garbage (2026-06-17, SHIPPED).** User: wall monitors (e.g. the "Transfer Bay" panel at `setviewpos -357 3939 -156 -87 1`) show a grid of dark BLOCKS over the GUI text ("checkerboard"); the assault-rifle ammo GUI grid "shows outside the bounds of the mask"; the muzzle flash "shows color but has a black background … how the renderer handles additive textures?". **ROOT CAUSE (one structural defect, NOT a regression — a 4-agent workflow `wf_1817a119` proved it: the blend7/clear/writeMask code is byte-identical across the last 8 commits, and the asset agent md5-matched grid/bg_mask2/bg2/bgwhite4 to the last git-tracked pak — the user just NOTICED it while combat-testing iter-120→123).** The WebGPU swapchain (drawn DIRECT, no offscreen blit — `st.texture` BGRA8 at RenderBackend_WebGPU.cpp:296/319) is **cleared with alpha=1.0** and EVERY pass pipeline writes alpha (`makeVariant` default `writeMask=All`, line 732; `blend.alpha`=`blend.color` at 758-759). The `gl_dst_alpha` overlays — `textures/sfx/monitor_glass2` (the CRT-grid glass, stage `(0.82,0.82,0.82,0.03)`, **pad1=7**) and the weapon-GUI `guis/assets/machinegun/grid` (pad1=7, gated by its preceding `bg_mask2` maskcolor pad1=6) — compute `src*dst.a + dst*(1-dst.a)`. With `dst.a≈1` everywhere the glass grid texture FULLY REPLACES the content → checkerboard; the ammo grid shows outside its mask; and an additive flash's surround reads as opaque black. Real GL leaves LOW framebuffer alpha on a monitor so the glass is faint (the 0.03 stage alpha is the tell it's meant to be barely-there). **FIX (the load-bearing change is the writeMask, not the clear — make framebuffer alpha a DEDICATED MASK CHANNEL written ONLY by the maskcolor/alpha-prime stage):** (1) `makeVariant` default `writeMask` → **RGB-only** `(Red|Green|Blue)` (line 732) so all 14 makeVariant pipelines (opaque/add/addAlpha/dstAlpha/invMul/dstAlphaAdd/invAlpha/mulAdd/alpha/filter + the 3 gui) stop touching alpha; `passAlphaPrimePipeline` keeps its explicit `WGPUColorWriteMask_Alpha` (line 791) and becomes the SOLE alpha writer; (2) the two non-makeVariant main-view pipelines — the opaque/ambient (line 466, One/Zero) and the lit/**interaction** (line 623, One/One) — also set RGB-only (else lit geometry behind a monitor leaves high alpha); (3) clear the swapchain alpha `1.0→0.0` (lines 3119/3261/3414) behind a NEW float cvar `r_wgpuAlphaClear`/`g_capWgpuAlphaClear` (default 0 = fix; the clear is a secondary baseline, the writeMask is what kills the block-causing non-uniform pollution); (4) **`cfg.alphaMode` `_Auto`→`_Opaque`** (line 190) — CRITICAL: the old alpha=1.0 masked a present-dependency; with alpha now ≈0 across the frame, Auto could resolve to premultiplied on **Safari/WebKit** and blank the whole canvas. Opaque guarantees the compositor IGNORES alpha on every browser (matches the GL path's `alpha:false`), so the alpha channel is purely internal. **VERIFIED LIVE (Chrome/Dawn, the confirmed repro):** the monitor checkerboard is GONE (post-rebuild vs pre-rebuild = 13.2 mean-abs-diff in the monitor region, text now readable on a uniform teal bg with only the legitimate faint CRT scanlines; monitor-interior contrast rose 20.4→31.1 stddev = text pops vs the muddy blocks). Frame stays opaque under Opaque (mean-L + bright HUD/lights/crosshair all intact, no transparency regression). The `r_wgpuAlphaClear` A/B barely moves (0.15-0.47 MAD) precisely BECAUSE the writeMask is the load-bearing fix — useful note: a cvar gating the wrong (secondary) half won't demonstrate a mechanism. BLAST RADIUS (low, by construction — only ALPHA writes are suppressed, RGB is byte-identical, present ignores alpha): glass-filter pad1=4 reads dst.COLOR not alpha (safe); decals 8/10/11 read dst.color (safe); dst-alpha 7/9 now read the mask-driven alpha (the fix); haze/bloom sample .rgb; determinism compares identical renders → IDENTICAL. **HONEST STATUS:** console checkerboard = DIRECTLY verified. Ammo-panel grid = the SAME pad1=6→7 dst-alpha gate (its maskcolor primes alpha right before the grid, now reading a clean 0 baseline elsewhere) — fixed by the identical mechanism, but I could not isolate a clean gun-GUI A/B. Muzzle-flash black-bg = could NOT capture a flash record (the deform-sprite `models/weapons/machinegun/machinegun_mflash`, `blend add`→pad1=2, only exists in the brief Fire() window; console `+attack` is suppressed by the wearable auto-fire so no flash fired under CDP) — agent C's medium-confidence read is the SAME alpha-pollution family (the leaking weapon-GUI dst-alpha grid, now masked, was painting dark over the flash region), but it is UNCONFIRMED; ask the user to verify ammo + flash on their display where they reproduce trivially. SUBVIEW writeMask sites (919/992/1071/1157/1201/1322/1446/1516 — offscreen monitor-mirrors/currentRender/detTex/bloom) deliberately LEFT as `WGPUColorWriteMask_All` — they're not the confirmed swapchain repro; treat as a scoped follow-up only if a monitor-MIRROR (camera-feed) regresses. Build emsdk-600; the fix is pure pipeline/clear state (no shader change → texture.wgsl untouched), so no runtime toggle for the writeMask — verify by rebuild. CROSS-BROWSER GATE (the Opaque assumption): confirm on Mac Safari + iOS-WebKit before the physical-iPhone reload.
