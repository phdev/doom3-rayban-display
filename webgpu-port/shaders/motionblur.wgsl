...
//  - textureSampleLevel is used so sampling is legal under conditional early-outs.
...
fn fs_main(@builtin(position) fc : vec4<f32>) -> @location(0) vec4<f32> {
  ...
  let uv = fc.xy / u.screenSize;
  let scene = textureSampleLevel(sceneTex, sceneSamp, uv, 0.0);
  if (u.zeroVel > 0.5 || u.scale < 0.01) { return scene; }

  var d = 0.5;
  if (u.noDepth < 0.5) { d = textureLoad(depthTex, vec2<i32>(fc.xy), 0); }
  if (d >= 0.9999) { return scene; }
  if (d < u.minDepth) { return scene; }

  let ndc = vec3<f32>(uv.x * 2.0 - 1.0, 1.0 - uv.y * 2.0, 2.0 * d - 1.0);
  let wH = u.invCurVP * vec4<f32>(ndc, 1.0);
  if (abs(wH.w) < 1e-6) { return scene; }
  let world = wH.xyz / wH.w;
  let pH = u.prevVP * vec4<f32>(world, 1.0);
  if (pH.w <= 1e-4) { return scene; }
  let pNdc = pH.xy / pH.w;
  let pUv = vec2<f32>(pNdc.x * 0.5 + 0.5, 0.5 - pNdc.y * 0.5);
  var velPx = (uv - pUv) * u.screenSize;
  if (!(dot(velPx, velPx) >= 0.0)) { return scene; }
  let len = length(velPx);
  if (len > u.maxOffsetPx) { velPx = velPx * (u.maxOffsetPx / len); }
  velPx = velPx * u.scale;
  if (length(velPx) < 0.5) { return scene; }
